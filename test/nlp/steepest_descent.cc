...
TEST(SteepestDescentTest, QuadratureFuncTests) {
    auto solver = mathlib::nlp::SteepestDescent();
    ...
    auto sol = solver.solve(0.0, std::move(cost_func), 1000, 1e-10, 1e-8, 1e-5);
    ASSERT_TRUE(sol);
    EXPECT_NEAR(sol.sol(), 0.5, 1e-5);
}

TEST(SteepestDescentTest, RosenbrockFuncTests) {
    ...
    auto sol = solver.solve(init, std::move(cost_func), 100000, 1e-6, 1e-6, 1e-4);
    ASSERT_TRUE(sol);
    for (auto i = 0u; i < n; ++i) {
